...
def train(opt):
    ...
    dataloader = DataLoader(
        ...
        shuffle=True,
        num_workers=0,  # Windows 환경에서는 0이 안전
        pin_memory=True,
        drop_last=True,
    ...
    if opt.use_id:
        ...
        if os.path.exists(id_weights_path):
            ...
            print(
                f"Warning: Identity Loss weights not found at {id_weights_path}"
            )
            criterionID = None

    criterionCycle = (
        CycleConsistencyLoss().to(device) if opt.use_cycle else None
    )
    ...
    for epoch in range(opt.epochs):
        ...
        for i, data in progress_bar:
            ...
            # G는 여기서 gradient 필요 없으므로 no_grad 로 fake 생성
            with torch.no_grad():
            ...
            for pred in pred_fake_G:
                loss_G_GAN = loss_G_GAN + criterionGAN(
                    pred, torch.ones_like(pred)
                )
            ...
            if criterionCycle is not None:
                loss_G_Cycle = (
                    criterionCycle(fake_img, target_maps, mask) * 10.0
                )
            ...
            if i % 100 == 0:
                ...
                if criterionVGG is not None:
                    writer.add_scalar(
                        "Loss/VGG", loss_G_VGG.item(), global_step
                    )
                if criterionCycle is not None:
                    writer.add_scalar(
                        "Loss/Cycle", loss_G_Cycle.item(), global_step
                    )
        ...
        if epoch % 5 == 0 or epoch == opt.epochs - 1:
            torch.save(
                netG.state_dict(),
                os.path.join(save_dir, f"netG_epoch_{epoch}.pth"),
            )
            torch.save(
                netD.state_dict(),
                os.path.join(save_dir, f"netD_epoch_{epoch}.pth"),
            )
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--name", type=str, default="experiment_1", help="Experiment name"
    )
    parser.add_argument(
        "--data_root",
        type=str,
